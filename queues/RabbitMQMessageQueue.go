...
import (
	...
	mqcon "github.com/pip-services3-gox/pip-services3-rabbitmq-gox/connect"
...
func (c *RabbitMQMessageQueue) Close(ctx context.Context, correlationId string) (err error) {
	...
	c.Logger.Trace(ctx, correlationId, "Closed queue %s", c.queue)
	return err
}
...
func (c *RabbitMQMessageQueue) Send(ctx context.Context, correlationId string, message *cqueues.MessageEnvelope) (err error) {
	...
	c.Logger.Debug(ctx, message.CorrelationId, "Sent message %s via %s", message, c.Name())
	return err
}
...
func (c *RabbitMQMessageQueue) Peek(ctx context.Context, correlationId string) (result *cqueues.MessageEnvelope, err error) {
	...
	if message != nil {
		c.Logger.Trace(ctx, message.CorrelationId, "Peeked message %s on %s", message, c.Name())
	}
...
func (c *RabbitMQMessageQueue) PeekBatch(ctx context.Context, correlationId string, messageCount int64) (result []*cqueues.MessageEnvelope, err error) {
	...
	c.Logger.Trace(ctx, correlationId, "Peeked %s messages on %s", len(messages), c.Name())
	return messages, err
}
...
func (c *RabbitMQMessageQueue) Abandon(ctx context.Context, message *cqueues.MessageEnvelope) (err error) {
	...
	if ok {
		...
		c.Logger.Trace(ctx, message.CorrelationId, "Abandoned message %s at %c", message, c.Name())
	}
	return nil
}
...
func (c *RabbitMQMessageQueue) Listen(ctx context.Context, correlationId string, receiver cqueues.IMessageReceiver) error {
	err := c.checkOpened("")
	if err != nil {
		c.Logger.Error(ctx, correlationId, err, "RabbitMQMessageQueue:Listen: Can't start listen "+err.Error())
		return nil
	}

	c.Logger.Debug(ctx, correlationId, "Started listening messages at %s", c.Name())
	...
	if err != nil {
		c.Logger.Error(ctx, correlationId, err, "RabbitMQMessageQueue:Listen: Can't consume to queue"+err.Error())
		return nil
	}
			...
			case msg := <-messageChannel:
				{
					...
					if recvErr != nil {
						c.Logger.Error(ctx, message.CorrelationId, recvErr, "Processing received message %s error in queue %s", message, c.Name())
					}
...
func (c *RabbitMQMessageQueue) Clear(ctx context.Context, correlationId string) (err error) {
	...
	c.Logger.Trace(ctx, correlationId, "Cleared  %s messages in queue %s", count, c.Name())
